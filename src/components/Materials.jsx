...
export default function Materials({ chips = defaultChips }) {
  return (
    <div className="flex flex-wrap justify-center gap-2">
      {chips.map((chip) => (
        <span
          key={chip.label}
          className="inline-flex items-center gap-1.5 bg-white border border-[#1B2D4F]/10 rounded-full px-3.5 py-1.5 font-display text-[13px] font-medium text-[#1B2D4F] shadow-sm"
        >
          <span className="text-[#E8882A]">{chip.icon}</span>
          {chip.label}
        </span>
      ))}
    </div>
